...
public class AnimalNames {
   ...
   */
    ...
    private static int count(String input) {
        List<String> contains = myContains(input);
        findMaxAnimals(contains,input);
        return 0;
    }
    private static int findMaxAnimals(List<String> fewAnimals,String input){
        int count=0;


        for (int i = 0; i < fewAnimals.size(); i++) {
            String[] tempArray = fewAnimals.get(i).split("");
            for (int j = 0; j < tempArray.length; j++) {
              input =   input.replaceFirst(tempArray[j],"");
            }
        }
    ...
    private static List<String> myContains(String input){
